<h4 class="text-center">User: <code><?= $username ?></code></h4>

<p><?= $this->tag->linkTo(['user/index', 'Log in']) ?>
    <?= $this->tag->linkTo(['user/logout', 'Log out']) ?>
    <?= $this->tag->linkTo(['user/change', 'Change password']) ?>
</p>
        ...
        <?php foreach ($data as $data) { ?>
            ...
            <?php if ($data['day'] == 0 || $data['day'] == 6 || $data['notWork'] != 0) { ?>
                ...
                <?php foreach ($data['tracking'] as $start) { ?>
                    ...
                    <?php } ?>
                    <?php if ($start['stopButtonStatus'] == 1 && $user_id == $start['id_user']) { ?>
                    <button type="button" class="btn btn-danger" id="stop_button" style="display: show" data-id="<?= $start['id'] ?>">Stop</button>
                    <?php } else { ?>
                    <?php if ($start['day'] == $data['number'] && $user_id == $start['id_user']) { ?>
