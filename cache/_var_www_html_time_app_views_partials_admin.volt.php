...
<p><?= $this->tag->linkTo(['user/logout', 'Log out']) ?><p>
<p></p>
<p><?= $this->tag->linkTo(['admin/late', 'Страница опозданий']) ?></p>
<p><?= $this->tag->linkTo(['admin/newuser', 'Добавление нового пользователя']) ?></p>
<p><?= $this->tag->linkTo(['admin/deleteuser', 'Удаление пользователя']) ?></p>
<p><?= $this->tag->linkTo(['admin/notWorkDays', 'Страница нерабочих дней']) ?></p>
        ...
        <?php foreach ($data as $data) { ?>
            ...
            <?php if ($data['day'] == 0 || $data['day'] == 6 || $data['notWork'] != 0) { ?>
                ...
                <?php foreach ($data['tracking'] as $start) { ?>
                    ...
                    <?php } ?>
                    <?php if ($start['stopButtonStatus'] == 1) { ?>
